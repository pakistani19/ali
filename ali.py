...
def menu():
    ...
    psb('         %s》%s》%s》%sWELLCOME TO AR TOOL%s《%s《%s《'%(M,H,B,H,B,H,M))
    print('%s '%(O))
...
def dump_name(_azimvau_):
    print('%s [%s•%s] %sEXAMPLE : AYESHA,KHAN'%(H,P,H,K));__name__ = _cici_azimvau_('%s [%s•%s] %sENTER NAME : '%(H,P,H,K)).split(',');_file_ = 'dump_name.json'
    try:os.remove(_file_)
    except:pass
...
def dump_username():
    print("%s [%s•%s] %sEXAMPLE : %sIMRAN KHAN"%(H,P,H,K,H));_query_ = _cici_azimvau_("%s [%s•%s] %sENTER NAME : "%(H,P,H,K)).lower()
...
def result():
    ...
    if ch in ['']:jalan('%s [%s!%s] %sINCORRECT CONTENT'%(M,P,M,P));menu()
        ...
        try:
            okl = os.listdir("AR-OK");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN OK FILE %s]'%(H,P,H));print('%s '%(O))
        ...
        try:
            cpl = os.listdir("AR-CP");print('%s '%(O));print('%s [%s CRACK RESULTS STORED IN CP FILES %s]'%(H,P,H));print('%s '%(O))
...
def cek_result():
    ...
    for memek in buka_baju:
        ...
        _format_ = "\r %s[%sAR-CP%s] %s          "%(K,P,K,kontol)
...
def genkey():
	try:
		...
		subprocess.check_output(["am", "start", "https://wa.me/+923047269778"])
		exit()
	except(IOError):
		rb = rx
		...
		banner()
		psb('       %s》%s》%s》%sPOWER OF AR%s《%s《%s《'%(M,H,B,H,B,H,M))
		print("\n\033[0;91m              YOU ARE NOT PREMIUM USER ")
		print("\033[0;91m           FIRST BUY PREMIUM KEY FROM OWNER")
		print("\033[0;91m.          PAYMENT CANT BE UNDONE")
		print("\033[0;91m.          CHANGE USER AGENT IF CP IDS CAME")
